...
\title{Federated Spellcasting Battle System}
\author{Haoran Li\quad Jiuen Feng\quad Yuhe Bian\quad Zhehao Chen}
\date{December 2025}
...
\begin{frame}{Task}
  \begin{itemize}
    ...
    \item Target task: classify IMU segments into multiple gestures / movements in ``Spell Game''.
    \item Federated learning on two boards, communication through BLE
    \item Constraint: training \textbf{directly on the board}
      with very limited RAM and compute. BLE transmitting limitation.
  \end{itemize}
\end{frame}
\begin{comment}
...
\begin{columns}[T,onlytextwidth]
  ...
  \begin{column}{0.45\textwidth}
    \centering
    \includegraphics[width=0.75\linewidth]{figures/raw_cbor_example.png}
    \vspace{0.3em}

    %{\scriptsize \textit{Raw time-series (one sample per class), 9 channels overlaid.}}
  \end{column}
\end{columns}
\end{frame}
\begin{comment}
...
\begin{frame}{Network Architecture on Nano 33 BLE}
  \begin{itemize}
    ...
    \[
      75 \rightarrow 32 \rightarrow \text{classes\_cnt}.
    \]
    \item Activation:
      \begin{itemize}
...
\begin{frame}{PC-Side Reference Training (PyTorch)}
  \begin{itemize}
    \item Use exactly the same 75-D feature vectors as on the Arduino:
      %\begin{itemize}
       % \item Dataset merged from all \texttt{.npz} files:
        %  about 180 training, 45 validation and 58 test samples, 4 classes.
        %\item Network on PC is identical to on-device network:
         % \[ 75 \rightarrow 64 \rightarrow \texttt{classes\_cnt} \]
        %\item Loss: cross-entropy, optimizer: SGD with \texttt{lr = 0.0015},
         % batch size 1 (to mimic on-device training).
      %\end{itemize}
    \item Results:
      \begin{itemize}
        \item After about 50 epochs, validation accuracy reaches
          \(\approx 95\%\).
        \item This PC model is used as an \textbf{upper-bound reference}
          for on-device learning performance.
      \end{itemize}
...
\begin{frame}
  ...
  \centering
  \includegraphics[width=0.9\textwidth]{figures/training_loss_acc.png}


  {\small \textit{PC-side centralized training reaches $\sim$95\% val.\ accuracy in $\sim$50 epochs.}}
\end{frame}
...
\begin{frame}{From Single Board to Federated Setup}
  \begin{itemize}
    \item We have  two Nano 33 BLE boards:
      \begin{itemize}
        \item Each board has its own local dataset from different users' gesture recordings.
      \end{itemize}
    \item Idea: In each round
      \begin{itemize}
      ...
      \end{itemize}
    \item \textbf{Only parameters are sent over BLE}.No raw sensor data is transmitted.
    \item  Our federated on-device training protocol is based on an open-source GitHub repository.
  \end{itemize}

  \vspace{0.6em}
\footnotesize
Reference: \href{https://github.com/niil87/Machine-Learning-for-IOT---Fall-2022-Batch-Lund-University}{\texttt{https://github.com/niil87/Machine-Learning-for-IOT---Fall-2022-Batch-Lund-University}}
\end{frame}
\begin{comment}
...
\begin{frame}{BLE Communication}
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.49\textwidth}
    \textbf{Board A: Leader (BLE Peripheral)}
    \begin{itemize}
      \item Advertises BLE service \texttt{MLLeader} and accepts a connection.
      \item \textbf{Sends} the current \textbf{global weights} over BLE (split into batches).
      \item \textbf{Receives} updated weights from Board B over BLE.
      \item Aggregates parameters  and starts the next round.
    \end{itemize}
  \end{column}

  \begin{column}{0.49\textwidth}
    \textbf{Board B: Worker (BLE Central)}
    \begin{itemize}
      \item Scans for \texttt{MLLeader}, connects, and subscribes to updates.
      \item \textbf{Receives} global weights over BLE and loads them into the NN.
      \item Runs \textbf{local on-device training} on its own IMU dataset.
      \item \textbf{Uploads} its updated weights back to Board A over BLE.
    \end{itemize}
  \end{column}
\end{columns}
\end{frame}
...
\begin{frame}
  \centering
  \includegraphics[width=\textwidth]{figures/diagram/BLEdiagram.pdf}
...
\begin{frame}{On-Device Training Curves}
\centering
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../BP_test_inference/output/acc_plot_byh.png}
    {\scriptsize Local Train with data\_byh}
  \end{column}
  \begin{column}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../BP_test_inference/output/acc_plot_fje.png}
    {\scriptsize Local Train with data\_fje}
  \end{column}
...
\scriptsize
\begin{itemize}
  \item Federated training improves \textbf{cross-user generalization} without sharing raw IMU data.
\end{itemize}
\end{frame}
...
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../distributed_test/output/plot_fd_leader.png}
    {\scriptsize Performance Comparison: Local vs. Federated Training on Leader}
  \end{column}
  \begin{column}{0.49\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../distributed_test/output/plot_fd_worker.png}
    {\scriptsize Performance Comparison: Local vs. Federated Training on Worker}
  \end{column}
\end{columns}
...
\begin{frame}{Confusion Matrices: Local Training one Board }
\centering
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.33\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../BP_test_inference/output/confusion_matrix_mbyh_dbyh.png}
    {\scriptsize Train byh \(\rightarrow\) Test byh\\acc = 82.50\%}
  \end{column}
  \begin{column}{0.33\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../BP_test_inference/output/confusion_matrix_mbyh_dczh.png}
    {\scriptsize Train byh \(\rightarrow\) Test czh\\ acc = 58.67\%}
  \end{column}
  \begin{column}{0.33\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../BP_test_inference/output/confusion_matrix_mbyh_dfje.png}
    {\scriptsize Train byh \(\rightarrow\) Test fje\\ acc =52.83\%}
  \end{column}
\end{columns}
...
\scriptsize
\begin{itemize}
  \item Best performance on \textbf{in-domain} test (A\(\rightarrow\)A).
  \item Accuracy drops on B/C due to \textbf{non-IID user motion patterns}.
\end{itemize}
\end{frame}
...
\begin{frame}{Confusion Matrices: Federated Training (fje+byh, FedAvg)}
\centering
\begin{columns}[T,onlytextwidth]
  \begin{column}{0.33\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../distributed_test/output/confusion_matrix_databyh.png}
    {\scriptsize FedAvg \(\rightarrow\) Test byh\\acc = 85.00\%}
  \end{column}

  \begin{column}{0.33\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../distributed_test/output/confusion_matrix_datafje.png}
    {\scriptsize FedAvg \(\rightarrow\) Test fje \\acc = 73.58\%}
  \end{column}
  ...
  \begin{column}{0.33\textwidth}
    \centering
    \includegraphics[width=\linewidth]{../distributed_test/output/confusion_matrix_dataczh.png}
    {\scriptsize FedAvg \(\rightarrow\) Test czh \\ acc = 62.67\%}
    \end{column}
  \end{columns}
...
\scriptsize
\begin{itemize}
  \item Federated model reduces cross-user confusion compared to local models.
  \item Gains come with communication cost (BLE weight exchange each round).
\end{itemize}
\end{frame}
...
\begin{comment}
\begin{frame}{Federated Training on PC (MPI + FedAvg)}
  \begin{itemize}
    \item Federated set-up (FedAvg):
      \begin{itemize}
        \item Each MPI rank $r=1,\dots,K$ loads its local
          \texttt{client\_r.npz} and trains a local model for
          \texttt{LOCAL\_EPOCHS} with SGD.
        \item Rank 0 acts as the server:
          \begin{itemize}
            \item broadcasts global weights to all clients,
            \item gathers updated weights,
            \item performs weighted averaging (FedAvg),
            \item evaluates on a held-out validation set and logs accuracy.
          \end{itemize}
        \item Hyper-parameters:\\
          \texttt{NUM\_ROUNDS = 50}, \texttt{LOCAL\_EPOCHS = 1},
          \texttt{BATCH\_SIZE = 1}, \texttt{LR = 0.0015}.
      \end{itemize}
  \end{itemize}
\end{frame}
...
\begin{frame}
  \centering
  \includegraphics[width=0.9\textwidth]{figures/fig_central_vs_fed.png}
\end{frame}
...
\begin{frame}{SpellGame Battle}
\begin{itemize}
  \item Two Nano 33 BLE boards (NODE\_A \& NODE\_B) perform \textbf{real-time gesture inference} on 2-second IMU windows.
  \item PC visualizes gestures and determines winner using rock-paper-scissors rules:
  \begin{center}
  circle > wave > peak > circle
  \end{center}
  \item Predicted gestures are transmitted to the \textbf{IMU Battle Visualizer} (PC-based GUI).
  \item PC displays:
  \begin{itemize}
    \item Both players' detected gestures simultaneously.
    \item \textbf{History log} of past matches.
    \item \textbf{Battle outcome} to verify inference accuracy and cross-user generalization.
  \end{itemize}
\end{itemize}
...
\begin{frame}{SpellGame Battle}
\textbf{Stability Issues Observed:}
\begin{itemize}
  \item Interface occasionally fails to \textbf{synchronize or display both gestures} simultaneously.
  \item \textbf{Hypothesized causes:}
  \begin{itemize}
    \item BLE packet delays disrupt real-time data flow.
    \item Temporal misalignment: users trigger gestures at different moments.
  \end{itemize}
\end{itemize}
\end{frame}
...
\vspace{0.4em}
\centering
\includegraphics[width=0.75\linewidth]{figures/visualizer.jpg}
...
\movie[externalviewer]{▶ Click to watch demo}{demo.mp4}
...
\begin{frame}{Summary}
  \begin{itemize}
    ...
    \item Implemented a small \textbf{on-device neural network}:
      \begin{itemize}
        \item 75--32--\texttt{classes\_cnt}, ReLU + softmax, SGD training.
      \end{itemize}
    \item Designed and tested a \textbf{federated learning prototype}:
      ...
      \end{itemize}
    \item Build a Federated Spellcasting Battle System 
  \end{itemize}
\end{frame}
